-- Make sure PowerPoint has an open presentation first
tell application "Microsoft Word"
	if not (exists active document) then
		display dialog "No active Word document found." buttons {"OK"} default button "OK" with icon stop
		return
	end if
...
on error errMsg number errNum
	display dialog "Couldn't click File ? Share ? Copy Link…" & return & return & ¬
		...
		¬
			"Check that PowerPoint is frontmost and that Accessibility permissions are granted in System Settings ? Privacy & Security ? Accessibility." buttons {"OK"} default button "OK" with icon stop
	return
end try
...
-- Build display name
if docName ends with ".docx" then
	set displayName to text 1 thru -6 of docName
else if docName ends with ".doc" then
