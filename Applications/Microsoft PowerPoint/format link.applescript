...
tell application "Microsoft PowerPoint"
	if not (exists active presentation) then
		display dialog "No active PowerPoint presentation found." buttons {"OK"} default button "OK" with icon stop
		return
	end if
...
on error errMsg number errNum
	display dialog "Couldn't click File ? Share ? Copy Link…" & return & return & ¬
		...
		"Check that PowerPoint is frontmost and that Accessibility permissions are granted in System Settings ? Privacy & Security ? Accessibility." ¬
...
-- Build display name
if docName ends with ".pptx" then
	set displayName to text 1 thru -6 of docName
else if docName ends with ".ppt" then
