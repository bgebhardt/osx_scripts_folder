...
tell application "Calendar"
	...
	set daysToFriday to my calcDaysToAdd(eStartDate)
	
	set newEndDate to (eEndDate + daysToFriday * days)
	set newStartDate to (eStartDate + daysToFriday * days)
...
-- Calculate how many days to add to get to Friday
on calcDaysToAdd(theDate)
	set daysToAdd to 0
	set dn to weekday of theDate
	if dn is Sunday then
		set daysToAdd to 5
	else if dn is Monday then
		set daysToAdd to 4
	else if dn is Tuesday then
		set daysToAdd to 3
	else if dn is Wednesday then
		set daysToAdd to 2
	else if dn is Thursday then
		set daysToAdd to 1
	else if dn is Friday then
		set daysToAdd to 0
	else if dn is Saturday then
		set daysToAdd to 6
	end if
	return daysToAdd
end calcDaysToAdd
